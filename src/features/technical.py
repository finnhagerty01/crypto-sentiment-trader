...
class TechnicalIndicators:
    ...
    def _add_macd(self, df: pd.DataFrame, close: pd.Series) -> pd.DataFrame:
        ...
        macd_above_prev = macd_above.shift(1).astype('boolean').fillna(False).astype(bool)
        df.loc[macd_above & ~macd_above_prev, 'macd_crossover'] = 1   # Bullish
        macd_above_prev_true = macd_above.shift(1).astype('boolean').fillna(True).astype(bool)
        df.loc[~macd_above & macd_above_prev_true, 'macd_crossover'] = -1  # Bearish
    ...
    def _add_moving_averages(self, df: pd.DataFrame, close: pd.Series) -> pd.DataFrame:
        ...
        sma20_above_prev_true = sma20_above_sma50.shift(1).astype('boolean').fillna(True).astype(bool)
        sma20_above_prev_false = sma20_above_sma50.shift(1).astype('boolean').fillna(False).astype(bool)
        df['golden_cross'] = (sma20_above_sma50 & ~sma20_above_prev_true).astype(int)
        df['death_cross'] = (~sma20_above_sma50 & sma20_above_prev_false).astype(int)
